...
if(isset($_GET['editar']) && isset($_GET['tabla']) && isset($_GET['id'])) {
    ...
    if ($datos): ?>
            ...
            <form method="POST" action="crud/actualizar.php" enctype="multipart/form-data">
                ...
                while ($campo = $resultado_estructura->fetch_assoc()):
                    ...
                    if($nombre_campo == "descripcion") {
                        echo "<textarea class='snow-editor' name='descripcion' rows='4'>" . htmlspecialchars($valor_actual) . "</textarea>";
                    }
                    else if($nombre_campo == "precio") {
                        echo "<input type='number' step='0.01' name='$nombre_campo' value='$valor_actual' required>";
                    }
                    else if($nombre_campo == "imagen" || $nombre_campo == "fondo") {
                        echo "<div class='imagen-container'>";
                        echo "<input type='file' name='$nombre_campo' accept='image/*'>";
                        if($valor_actual) {
                            echo "<div class='preview-imagen'>";
                            echo "<img src='../static/$valor_actual' style='max-width:200px'>";
                            echo "<button type='button' class='btn-quitar-imagen' data-campo='$nombre_campo'>Quitar imagen</button>";
                            echo "</div>";
                            echo "<input type='hidden' name='quitar_" . $nombre_campo . "' value='0'>";
                        }
                        echo "</div>";
                    }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const botonesQuitar = document.querySelectorAll('.btn-quitar-imagen');
            botonesQuitar.forEach(boton => {
                boton.addEventListener('click', function() {
                    const campo = this.dataset.campo;
                    const container = this.closest('.imagen-container');
                    const preview = container.querySelector('.preview-imagen');
                    const inputQuitar = container.querySelector(`input[name='quitar_${campo}']`);
                    if (preview) preview.style.display = 'none';
                    if (inputQuitar) inputQuitar.value = '1';
                });
            });

            const selectImagen2 = document.querySelector('select[name="imagen2"]');
            const previewImagen2 = document.querySelector('.preview-imagen2');

            if(selectImagen2) {
                selectImagen2.addEventListener('change', function() {
                    const selectedImage = this.value;
                    if(selectedImage && previewImagen2) {
                        previewImagen2.innerHTML = `<img src="../static/${selectedImage}" style="max-width:200px">`;
                    }
                });
            }
        });
