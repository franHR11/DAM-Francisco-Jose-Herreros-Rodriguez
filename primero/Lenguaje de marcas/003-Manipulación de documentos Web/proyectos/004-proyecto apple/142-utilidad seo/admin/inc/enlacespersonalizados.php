...
while ($enlace = $resultadoEnlaces->fetch_assoc()) {
    ...
    // Modificado para usar data-url en lugar de href directo
    echo "<a href='#' data-url='" . htmlspecialchars($enlace['url']) . "' onclick='cargarEnlace(\"" . htmlspecialchars($enlace['url']) . "\"); return false;'>" 
...
function cargarEnlace(url) {
    // Cargar el contenido en la sección principal
    const section = document.querySelector('section');
    section.innerHTML = `<iframe src="${url}" style="width:100%; height:100%; border:none;"></iframe>`;
    
    // Actualizar la URL sin recargar la página
    const newUrl = new URL(window.location.href);
    newUrl.searchParams.set('enlace_custom', encodeURIComponent(url));
